...
def boXValueFetcher(roomid):
    ...
    #****dbcon-1****
    database=mycon.connect(host='example.com',user='YourUser',password='YourPassword',database='YourDatabaseName')
...
def afterMatchScreen(msg,roomid):#msg-message to be printed
    ...
    cmd="UPDATE roomlist SET status='unused' WHERE room='%s'"%(roomid)
    cursor.execute(cmd)
    database.commit()
...
class frontPageFuncs:
    ...
    def createRoom():
        ...
        cmd="SELECT room FROM roomlist WHERE status='unused'"
        cursor.execute(cmd)
        ...
        cmd="UPDATE roomlist SET status='used' , time='%s' WHERE room='%s'"%(datetime,roomid)
        cursor.execute(cmd)
        database.commit()
...
#__main__
#os.system("title "+'multiplayer-tictactoe') #only for windows version
try:
    #****dbcon-4****
    database=mycon.connect(host='example.com',user='YourUser',password='YourPassword',database='YourDatabaseName')
    cursor=database.cursor()
